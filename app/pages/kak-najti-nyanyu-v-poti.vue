      ...
      <div class="max-w-4xl mx-auto">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-8">
          ...
          <div class="prose prose-gray dark:prose-invert max-w-none">
                ...
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-1">
                      {{ $t('pages.findNanny.poti.steps.step1.name') }}
                    </h3>
                    <p class="text-gray-700 dark:text-gray-300">
                      {{ $t('pages.findNanny.poti.steps.step1.text') }}
                    </p>
                  </div>
                </div>
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-1">
                      {{ $t('pages.findNanny.poti.steps.step2.name') }}
                    </h3>
                    <p class="text-gray-700 dark:text-gray-300">
                      {{ $t('pages.findNanny.poti.steps.step2.text') }}
                    </p>
                  </div>
                </div>
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-1">
                      {{ $t('pages.findNanny.poti.steps.step3.name') }}
                    </h3>
                    <p class="text-gray-700 dark:text-gray-300">
                      {{ $t('pages.findNanny.poti.steps.step3.text') }}
                    </p>
                  </div>
                </div>
                <div class="flex gap-4">
                  ...
                  <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white mb-1">
                      {{ $t('pages.findNanny.poti.steps.step4.name') }}
                    </h3>
                    <p class="text-gray-700 dark:text-gray-300">
                      {{ $t('pages.findNanny.poti.steps.step4.text') }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section class="mb-8">
              <h2 class="text-2xl font-semibold text-gray-900 dark:text-white mb-4">
                {{ $t('pages.findNanny.poti.considerationsTitle') }}
              </h2>
            ...
            <section class="mb-8">
              <div class="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-6 text-center">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                  {{ $t('pages.findNanny.poti.ctaTitle') }}
                </h3>
                <UButton to="/search?city=poti" color="primary" size="lg">
                  {{ $t('pages.findNanny.poti.ctaButton') }}
                </UButton>
              </div>
            </section>
          </div>
...
const pageTitle = t('pages.findNanny.poti.title')
const pageDescription = t('pages.findNanny.poti.intro')
...
const considerations = computed(() => {
  try {
    ...
    if (messages?.pages?.findNanny?.poti?.considerations) {
      const value = messages.pages.findNanny.poti.considerations
      if (Array.isArray(value)) {
        if (value.length > 0 && typeof value[0] === 'string') {
          return value
        }
        return value.map((_, index) => {
          const key = `pages.findNanny.poti.considerations.${index}`
