...
const advantages = computed(() => {
  try {
    ...
    if (messages?.pages?.findBabysitter?.poti?.advantages) {
      const value = messages.pages.findBabysitter.poti.advantages
      if (Array.isArray(value)) {
        if (value.length > 0 && typeof value[0] === 'string') {
          return value
        }
        return value.map((_, index) => {
          const key = `pages.findBabysitter.poti.advantages.${index}`
